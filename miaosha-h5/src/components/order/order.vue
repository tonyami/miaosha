<template>
  <div class="order">
    ...
    <div class="info border">
      ...
      <div class="row" v-show="order.status === -1">
        关闭时间：{{ order.updateTime }}
      </div>
    </div>
    <van-submit-bar
...
export default {
  ...
  methods: {
    ...
    cancel() {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          this.$api
            .cancelOrder({ orderId: this.order.orderId })
            .then((res) => {
              this.disabled = false
    ...
    getOrder() {
      this.$api
        .getOrder({ orderId: this.order.orderId })
        .then((res) => {
          this.order = res
        })
        .catch((_) => {})
